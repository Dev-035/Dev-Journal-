...
function loadScreenshot(file) {
  ...
  reader.onload = ev => {
    ...
    document.getElementById('ssLabel').textContent = '✓ Screenshot';
  };
...
function resetForm() {
  ...
  const ssLbl = document.getElementById('ssLabel');
  if (ssLbl) ssLbl.textContent = 'Screenshot';
  setOutcome('profit');
  setSide('buy');
  onMarketChange();
...
function renderSettings() {
  ...
  list.innerHTML = filtered.map(t => {
    ...
    return [
      '<div class="stt-row stt-trade-row">',
        ...
        '<div class="stt-trade-right">',
          ...
          '<button class="stt-del-btn" onclick="deleteTradeFromSettings(' + t.id + ')">🗑</button>',
        '</div>',
      '</div>'
    ].join('');
  }).join('');
