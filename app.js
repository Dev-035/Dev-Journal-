...
function submitAuth(e) {
  ...
  if (loginMode === 'login') {
    auth.signInWithEmailAndPassword(email, password).catch(err => {
      ...
      // Firebase returns same error for wrong password and user not found (security feature)
      if (err.code === 'auth/wrong-password' || 
        ...
        errorEl.textContent = 'Network error. Check your internet connection';
      } else {
        // Fallback - if it's any other auth error, assume wrong credentials
        errorEl.textContent = 'Invalid email or password';
      }
    });
  } else {
    ...
    auth.createUserWithEmailAndPassword(email, password)
      ...
      .catch(err => {
        ...
        if (err.code === 'auth/email-already-in-use') {
          ...
          errorEl.textContent = 'Password is too weak. Use at least 6 characters';
        } else if (err.code === 'auth/network-request-failed') {
          errorEl.textContent = 'Network error. Check your internet connection';
        } else {
...
function changePassword() {
  if (!currentUser) {
    alert('You are in guest mode. Please sign in with an account to change password.');
    return;
  }
...
function sendPasswordResetEmail() {
  if (!currentUser) {
    alert('You are in guest mode. Please sign in with an account to reset password.');
    return;
  }
  
  const email = currentUser.email;
  if (confirm(`Send password reset email to ${email}?\n\nYou will receive an email with a link to reset your password. After resetting, you'll need to sign in again with your new password.`)) {
    // Show loading state
    const originalMsg = 'Sending email...';
    alert(originalMsg);
    ...
    auth.sendPasswordResetEmail(email)
      ...
      .catch(err => {
        ...
        if (err.code === 'auth/too-many-requests') {
          errorMsg += 'Too many requests. Please try again later (wait 15-30 minutes).';
        } else if (err.code === 'auth/user-not-found') {
          errorMsg += 'User account not found. Please contact support.';
        } else if (err.code === 'auth/invalid-email') {
...
function buildJournalCard(t, showDelete) {
  ...
  // Check if trade has any screenshots
  const hasScreenshots = t.screenshot1 || t.screenshot2;
  const screenshotCount = (t.screenshot1 ? 1 : 0) + (t.screenshot2 ? 1 : 0);
      ...
      '<div class="jnl-card-main">',
        '<div class="jnl-card-top">',
          ...
          '<div class="jnl-card-right">',
            ...
            hasScreenshots ? '<div class="jnl-has-ss">📷' + (screenshotCount > 1 ? ' ' + screenshotCount : '') + '</div>' : '',
          '</div>',
        '</div>',
      '</div>',
    '</div>'
...
function openTradeDetail(id) {
  ...
  let screenshotsHtml = '';
  if (t.screenshot1 || t.screenshot2) {
    screenshotsHtml = '<div class="td-section-title">Screenshots</div><div class="td-screenshots-grid">';
    if (t.screenshot1) {
      screenshotsHtml += '<img class="td-screenshot" src="' + t.screenshot1 + '" alt="screenshot 1" onclick="openModalWithImage(\'' + t.screenshot1 + '\')" />';
    }
    if (t.screenshot2) {
      screenshotsHtml += '<img class="td-screenshot" src="' + t.screenshot2 + '" alt="screenshot 2" onclick="openModalWithImage(\'' + t.screenshot2 + '\')" />';
    }
    screenshotsHtml += '</div>';
  }
...
function renderSettings() {
  ...
  const nameEl = document.getElementById('stt-user-name');
  const emailEl = document.getElementById('stt-user-email');
  const badgeEl = document.getElementById('stt-storage-badge');
  ...
  if (currentUser) {
    ...
    if (badgeEl) {
      badgeEl.textContent = '☁️ Cloud Storage';
      badgeEl.className = 'profile-badge cloud';
    }
    if (avatarEl) avatarEl.textContent = displayName.charAt(0).toUpperCase();
    
    // Show security section for logged-in users
    ...
    if (badgeEl) {
      badgeEl.textContent = '💾 Local Storage';
      badgeEl.className = 'profile-badge';
    }
    if (avatarEl) avatarEl.textContent = 'G';
    
    // Hide security section for guest users
...
function deleteSelectedTrades() {
  ...
  const batch = db.batch();
  selectedTrades.forEach(id => {
    batch.delete(tradesRef().doc(String(id)));
  });
  batch.commit().then(() => {
    exitSelectionMode();
  });
